...
export function AdminDoctors({ setScreen }: Props) {
  ...
  const filtered = doctors.filter(d => d.name.includes(search) || d.id.includes(search));
    ...
    <div className="admin-dash-screen" style={{ background: bg, fontSize: `${fontSize}px`, minHeight: '100vh' }}>
      ...
      <div className="admin-dash-content">
        ...
        <div className="admin-search-bar" dir="rtl">
          ...
          <input type="text" placeholder="ابحث عن طبيب" value={search}
        ...
        <div className="admin-patients-list">
          {filtered.map(d => (
            <div key={d.id} className="admin-patient-card" dir="rtl" style={{ background: cardBg, borderColor: border }}>
              <img src={d.avatar} alt={d.name} className="admin-patient-avatar" />
              <div className="admin-patient-info" style={{ alignItems: 'center', cursor: 'pointer' }}
                onClick={() => setScreen('admin-doctor-detail')}>
                <span className="admin-patient-name" style={{ fontSize, color: textMain }}>{d.name}</span>
                <span className="admin-patient-phone" style={{ marginTop: 0, fontWeight: 600, color: textSub }}>{d.specialty}</span>
                <span className="admin-patient-id" style={{ marginTop: 4, color: textSub }}>{d.id} ID</span>
                <span className="admin-patient-phone" style={{ marginTop: 0, color: textSub, fontSize: fontSize - 1 }}>
                  {d.phone}
