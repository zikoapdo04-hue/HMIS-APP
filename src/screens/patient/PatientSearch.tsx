...
const NAV_ITEMS = [
  { key: 'patient-profile', label: 'حسابي',    labelEn: 'Profile', icon: (c: string) => <svg width="22" height="22" viewBox="0 0 24 24" fill={c}><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg> },
  { key: 'patient-appointments', label: 'المواعيد', labelEn: 'Appts', icon: (c: string) => <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg> },
  { key: 'search',            label: 'بحث',      labelEn: 'Search', active: true, icon: (c: string) => <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg> },
  { key: 'patient-home',     label: 'الرئيسية', labelEn: 'Home', icon: (c: string) => <svg width="22" height="22" viewBox="0 0 24 24" fill={c}><path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/></svg> },
] as const
...
export function PatientSearch({ setScreen, onSelectDoctor }: Props) {
  ...
  const results = query.trim().length > 0
    ? allDoctors.filter(d => {
        const specName = getSpecialtyName(d.specialty);
        return d.name.toLowerCase().includes(query.toLowerCase()) || specName.toLowerCase().includes(query.toLowerCase());
      })
    : allDoctors;

  const handleSelect = (d: UserModel) => {
    onSelectDoctor?.({ uid: d.id, name: d.name, specialty: getSpecialtyName(d.specialty), rating: d.rating ?? 0, phone: d.phone, address: d.hospitalAddress ?? '', avatar: d.imageUrl ?? undefined });
  ...
  return (
    <div style={{ background: COLORS.bg, height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <header style={{ background: GRADIENTS.heroBg, padding: '20px 20px 24px', flexShrink: 0, boxShadow: SHADOWS.header, borderRadius: `0 0 ${RADIUS.xxl} ${RADIUS.xxl}` }}>
        <p style={{ fontFamily: FONT.cairo, fontSize: 13, color: 'rgba(255,255,255,0.75)', margin: '0 0 4px', textAlign: lang === 'ar' ? 'right' : 'left' }}>{lang === 'ar' ? 'اعثر على طبيبك' : 'Find Your Doctor'}</p>
        <h2 style={{ fontFamily: FONT.cairo, fontSize: 24, fontWeight: 800, color: 'white', margin: '0 0 16px', textAlign: lang === 'ar' ? 'right' : 'left' }}>{lang === 'ar' ? 'ابحث عن طبيب' : 'Search for a Doctor'}</h2>
        {/* Search box */}
        <div style={{ display: 'flex', alignItems: 'center', background: 'rgba(255,255,255,0.18)', borderRadius: RADIUS.lg, padding: '4px 8px 4px 4px', border: '1.5px solid rgba(255,255,255,0.28)', backdropFilter: 'blur(8px)' }} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
          <input
            type="text"
            placeholder={lang === 'ar' ? 'اسم الطبيب أو التخصص...' : 'Doctor name or specialty...'}
            value={query}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 16px 8px' }}>
        {loading && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', paddingTop: 60 }}>
            ...
            <p style={{ fontFamily: FONT.cairo, color: COLORS.textMuted, marginTop: 16 }}>{lang === 'ar' ? 'جاري البحث...' : 'Searching...'}</p>
          </div>
        )}
        ...
        {!loading && results.length === 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', paddingTop: 60, gap: 12 }}>
            ...
            <p style={{ fontFamily: FONT.cairo, color: COLORS.textMuted, fontSize: 16, fontWeight: 600 }}>{lang === 'ar' ? 'لا توجد نتائج' : 'No results found'}</p>
          </div>
        )}

        {!loading && results.length > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
            <p style={{ fontFamily: FONT.cairo, fontSize: 13, color: COLORS.textMuted, margin: '0 4px 4px' }}>{results.length} {lang === 'ar' ? 'طبيب متاح' : 'Available Doctors'}</p>
            {results.map(dr => (
              <button
                key={dr.id}
                ...
                style={{
                  ...
                  transition: 'transform 0.18s, box-shadow 0.18s', width: '100%', textAlign: lang === 'ar' ? 'right' : 'left',
                }}
              ...
              >
                ...
                />
                <div style={{ flex: 1 }}>
                  <p style={{ fontFamily: FONT.cairo, fontSize: 17, fontWeight: 800, color: COLORS.textPrimary, margin: '0 0 3px' }}>{lang === 'ar' ? `د. ${dr.name}` : `Dr. ${dr.name}`}</p>
                  <p style={{ fontFamily: FONT.cairo, fontSize: 13, fontWeight: 600, color: COLORS.primary, margin: '0 0 6px' }}>{getSpecialtyName(dr.specialty)}</p>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-around', padding: '12px 8px 16px', background: COLORS.navBg, borderTop: `1.5px solid ${COLORS.navBorder}`, boxShadow: SHADOWS.nav, flexShrink: 0 }}>
        {NAV_ITEMS.map(item => {
          ...
          return (
            <button key={item.key} onClick={() => !active && setScreen(item.key as Screen)} style={{ background: 'none', border: 'none', cursor: active ? 'default' : 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
              <div style={{ width: 44, height: 44, borderRadius: RADIUS.sm, background: active ? COLORS.primaryLight : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {item.icon(color)}
              </div>
              <span style={{ fontFamily: FONT.cairo, fontSize: 11, fontWeight: active ? 700 : 600, color }}>{lang === 'ar' ? item.label : item.labelEn}</span>
            </button>
          );
        })}
      </nav>
