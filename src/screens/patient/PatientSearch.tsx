...
const NAV_ITEMS = [
  ...
  { key: 'search',            label: 'بحث',      active: true, icon: (c: string) => <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg> },
...
export function PatientSearch({ setScreen, onSelectDoctor }: Props) {
  ...
  const results = query.trim().length > 0
    ? allDoctors.filter(d => d.name.includes(query) || (d.specialty ?? '').includes(query))
    : allDoctors;
  ...
  return (
    <div style={{ background: COLORS.bg, height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <header style={{ background: GRADIENTS.heroBg, padding: '20px 20px 24px', flexShrink: 0, boxShadow: SHADOWS.header, borderRadius: `0 0 ${RADIUS.xxl} ${RADIUS.xxl}` }}>
        <p style={{ fontFamily: FONT.cairo, fontSize: 13, color: 'rgba(255,255,255,0.75)', margin: '0 0 4px', textAlign: 'right' }}>اعثر على طبيبك</p>
        <h2 style={{ fontFamily: FONT.cairo, fontSize: 24, fontWeight: 800, color: 'white', margin: '0 0 16px', textAlign: 'right' }}>ابحث عن طبيب</h2>
        {/* Search box */}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 16px 8px' }}>
        {loading && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', paddingTop: 60 }}>
            ...
            <p style={{ fontFamily: FONT.cairo, color: COLORS.textMuted, marginTop: 16 }}>جاري البحث...</p>
          </div>
        )}
        ...
        {!loading && results.length > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }} dir="rtl">
            <p style={{ fontFamily: FONT.cairo, fontSize: 13, color: COLORS.textMuted, margin: '0 4px 4px' }}>{results.length} طبيب متاح</p>
            {results.map(dr => (
              <button
                key={dr.id}
        ...
        {NAV_ITEMS.map(item => {
          ...
          return (
            <button key={item.key} onClick={() => !active && setScreen(item.key as Screen)} style={{ background: 'none', border: 'none', cursor: active ? 'default' : 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
              ...
              <span style={{ fontFamily: FONT.cairo, fontSize: 11, fontWeight: active ? 700 : 600, color }}>{item.label}</span>
            </button>
          );
        })}
      </nav>
