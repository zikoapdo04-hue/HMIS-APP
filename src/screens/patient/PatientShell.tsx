...
const STATUS_MAP: Record<string, { labelAr: string; labelEn: string; bg: string; color: string }> = {
  [AppointmentStatus.Pending]:   { labelAr: 'في الانتظار', labelEn: 'Pending',   bg: '#FEF3C7', color: '#92400E' },
  [AppointmentStatus.Confirmed]: { labelAr: 'مؤكد',        labelEn: 'Confirmed', bg: '#D1FAE5', color: '#065F46' },
}
...
function HomeTab({ setScreen, setSpecialty }: { setScreen: (s: Screen) => void; setSpecialty: (s: string) => void }) {
  ...
  const { specialties, loading: specLoading } = useSpecialtyContext();
          ...
          {specLoading ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(88px, 1fr))', gap: 8 }}>
              {specialties.map((s) => (
                <button
                  key={s.id}
                  onClick={() => { setSpecialty(s.id); setScreen('clinics'); }}
            ...
            {results.map(dr => (
              <button key={dr.id} onClick={() => handleSelect(dr)}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ fontFamily: FONT.cairo, fontSize: 12, fontWeight: 600, color: COLORS.primary, margin: '0 0 4px' }}>{getSpecialtyName(dr.specialty)}</p>
            ...
            return (
              <div key={appt.id} style={{ background: COLORS.card, border: `1.5px solid ${COLORS.cardBorder}`, borderRadius: RADIUS.xl, padding: '14px 14px', boxShadow: SHADOWS.card }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                  ...
                  <span style={{ fontFamily: FONT.cairo, fontSize: 11, fontWeight: 700, background: st.bg, color: st.color, borderRadius: RADIUS.pill, padding: '3px 8px', flexShrink: 0 }}>{lang === 'ar' ? st.labelAr : st.labelEn}</span>
                </div>
              ...
              <div key={apt.id} style={{ background: COLORS.card, border: `1.5px solid ${COLORS.cardBorder}`, borderRadius: RADIUS.xl, padding: '14px', boxShadow: SHADOWS.card }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <p style={{ fontFamily: FONT.cairo, fontSize: 12, fontWeight: 600, color: COLORS.primary, margin: '0 0 2px' }}>{getSpecialtyName(apt.doctorSpecialty)}</p>
              ...
              <div key={rec.id} style={{ background: COLORS.card, border: `1.5px solid ${COLORS.cardBorder}`, borderRadius: RADIUS.xl, padding: '14px', boxShadow: SHADOWS.card }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <p style={{ fontFamily: FONT.cairo, fontSize: 12, fontWeight: 600, color: COLORS.primary, margin: '0 0 2px' }}>{getSpecialtyName(rec.specialty)}</p>
...
export function PatientShell({ setScreen, initialTab = 'home', setSpecialty, onSelectDoctor }: Props) {
  ...
  return (
    <div style={{ background: COLORS.bg, height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-around', padding: '10px 8px 14px', background: COLORS.navBg, borderTop: `1.5px solid ${COLORS.navBorder}`, boxShadow: SHADOWS.nav, flexShrink: 0 }} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
        {NAV.map(item => {
