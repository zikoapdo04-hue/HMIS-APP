...
export function Clinics({ setScreen, onBack, onSelectDoctor, specialty }: Props) {
  ...
  const filtered = (() => {
    // specialty prop is a specialty ID (e.g. "cardiology") — match against d.specialty (also an ID)
    let list = specialty
      ? allDoctors.filter(d => d.specialty === specialty)
      : allDoctors;
    if (search.trim()) {
      const q = search.toLowerCase();
      list = list.filter(d => d.name.toLowerCase().includes(q) || getSpecialtyName(d.specialty).toLowerCase().includes(q));
    }
    return list;
  })();

  const handleSelect = (dr: UserModel) => {
    const info: DoctorInfo = { uid: dr.id, name: dr.name, specialty: getSpecialtyName(dr.specialty), rating: dr.rating ?? 0, phone: dr.phone, address: dr.hospitalAddress ?? '', avatar: dr.imageUrl ?? undefined };
    onSelectDoctor?.(info);
  ...
  return (
    <div style={{ background: COLORS.bg, height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
          ...
          <div dir={lang === 'ar' ? 'rtl' : 'ltr'}>
            <p style={{ fontFamily: FONT.cairo, fontSize: 13, color: 'rgba(255,255,255,0.75)', margin: 0 }}>
              {specialty ? (lang === 'ar' ? `تخصص ${specialty}` : `${specialty} Specialty`) : (lang === 'ar' ? 'جميع الأطباء' : 'All Doctors')}
            </p>
            <h2 style={{ fontFamily: FONT.cairo, fontSize: 22, fontWeight: 800, color: 'white', margin: 0 }}>
              {specialty ? (lang === 'ar' ? `أطباء ${specialty}` : `${specialty} Doctors`) : (lang === 'ar' ? 'الأطباء' : 'Doctors')}
            </h2>
          </div>
        </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 16px 8px' }}>
        {loading && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', paddingTop: 60 }}>
            ...
            <p style={{ fontFamily: FONT.cairo, color: COLORS.textMuted, marginTop: 16 }}>{lang === 'ar' ? 'جاري التحميل...' : 'Loading...'}</p>
          </div>
        )}
        ...
        {!loading && filtered.length > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
            <p style={{ fontFamily: FONT.cairo, fontSize: 13, color: COLORS.textMuted, margin: '0 4px 4px' }}>{filtered.length} {lang === 'ar' ? 'طبيب متاح' : 'doctors available'}</p>
            {filtered.map(dr => (
              <button
                key={dr.id}
      ...
      {/* Bottom Nav */}
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-around', padding: '12px 8px 16px', background: COLORS.navBg, borderTop: `1.5px solid ${COLORS.navBorder}`, boxShadow: SHADOWS.nav, flexShrink: 0 }} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
        {[
          { key: 'patient-home',         labelAr: 'الرئيسية', labelEn: 'Home',  icon: (c: string) => <svg width="22" height="22" viewBox="0 0 24 24" fill={c}><path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/></svg> },
          { key: 'patient-search',       labelAr: 'بحث',       labelEn: 'Search', icon: (c: string) => <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg> },
          { key: 'patient-appointments', labelAr: 'المواعيد',  labelEn: 'Appointments', icon: (c: string) => <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg> },
          { key: 'patient-profile',      labelAr: 'حسابي',    labelEn: 'Profile', icon: (c: string) => <svg width="22" height="22" viewBox="0 0 24 24" fill={c}><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg> },
        ].map(item => {
          ...
          return (
            <button key={item.key} onClick={() => setScreen(item.key as Screen)} style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
              ...
              <span style={{ fontFamily: FONT.cairo, fontSize: 11, fontWeight: 600, color }}>{lang === 'ar' ? item.labelAr : item.labelEn}</span>
            </button>
          );
        })}
      </nav>
