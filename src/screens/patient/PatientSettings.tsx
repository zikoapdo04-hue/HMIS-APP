      ...
      <header style={{ background: GRADIENTS.heroBg, padding: '20px 20px 28px', flexShrink: 0, boxShadow: SHADOWS.header, borderRadius: `0 0 ${RADIUS.xxl} ${RADIUS.xxl}` }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ textAlign: 'center' }}>
            <p style={{ fontFamily: FONT.cairo, fontSize: 12, color: 'rgba(255,255,255,0.7)', margin: 0 }}>تخصيص تجربتك</p>
            <h2 style={{ fontFamily: FONT.cairo, fontSize: 22, fontWeight: 800, color: 'white', margin: 0 }}>الإعدادات</h2>
          </div>
          <div style={{ width: 40 }} />
        </div>
      </header>
      ...
      <div style={{ flex: 1, padding: '24px 16px', display: 'flex', flexDirection: 'column', gap: 12, overflowY: 'auto' }} dir="rtl">
        {SETTINGS_ITEMS.map((item, i) => (
          ...
          >
            ...
            <span style={{ flex: 1, fontFamily: FONT.cairo, fontSize: 16, fontWeight: 700, color: COLORS.textPrimary, textAlign: 'right' }}>{item.label}</span>
            {item.action && (
