  ...
  return (
    <div style={{ background: COLORS.bg, height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ background: GRADIENTS.heroBg, padding: '20px 20px 32px', flexShrink: 0, position: 'relative' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
          <img
            src={avatarSrc}
            alt={doctor.name}
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 3 }}>
              ...
              <span style={{ fontFamily: FONT.inter, fontSize: 13, fontWeight: 700, color: 'rgba(255,255,255,0.85)', [lang === 'ar' ? 'marginRight' : 'marginLeft']: 4 }}>{doctor.rating}/5</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px 16px 24px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
          ...
          <div style={{ background: COLORS.card, border: `1.5px solid ${COLORS.cardBorder}`, borderRadius: RADIUS.xl, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 14, boxShadow: SHADOWS.card }}>
            ...
            <div>
              <p style={{ fontFamily: FONT.cairo, fontSize: 12, color: COLORS.textMuted, margin: '0 0 2px', fontWeight: 600 }}>{lang === 'ar' ? 'رقم التواصل' : 'Contact Number'}</p>
          ...
          {doctor.address && (
            <div style={{ background: COLORS.card, border: `1.5px solid ${COLORS.cardBorder}`, borderRadius: RADIUS.xl, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 14, boxShadow: SHADOWS.card }}>
              ...
              <div>
                <p style={{ fontFamily: FONT.cairo, fontSize: 12, color: COLORS.textMuted, margin: '0 0 2px', fontWeight: 600 }}>{lang === 'ar' ? 'موقع العيادة' : 'Clinic Location'}</p>
          ...
          <div style={{ background: COLORS.card, border: `1.5px solid ${COLORS.cardBorder}`, borderRadius: RADIUS.xl, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 14, boxShadow: SHADOWS.card }}>
            ...
            <div>
              <p style={{ fontFamily: FONT.cairo, fontSize: 12, color: COLORS.textMuted, margin: '0 0 2px', fontWeight: 600 }}>{lang === 'ar' ? 'التخصص' : 'Specialty'}</p>
