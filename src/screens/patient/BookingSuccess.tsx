...
export function BookingSuccess({ setScreen }: Props) {
  ...
  return (
    <div style={{ background: COLORS.bg, height: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '32px', overflow: 'hidden' }}>
      ...
      <h2 style={{ fontFamily: FONT.cairo, fontSize: 26, fontWeight: 800, color: COLORS.textPrimary, margin: '0 0 12px', textAlign: 'center' }} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
        {lang === 'ar' ? 'تم الحجز بنجاح!' : 'Booking Successful!'}
      </h2>
      <p style={{ fontFamily: FONT.cairo, fontSize: 15, fontWeight: 500, color: COLORS.textSec, margin: '0 0 48px', textAlign: 'center', lineHeight: 1.6 }} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
        {lang === 'ar' ? 'سيتم تأكيد موعدك قريباً' : 'Your appointment will be confirmed soon'}<br/>
        {lang === 'ar' ? 'يمكنك متابعة حجزك من قسم المواعيد' : 'You can track it from the appointments section'}
      </p>
