...
export function Login({ setScreen }: Props) {
  ...
  const handleLogin = async () => {
    if (!email || !password) { setError(t('الرجاء إدخال جميع البيانات', 'Please enter all details')); return; }
    setLoading(true);
    setError('');
    try {
      const user = await login(email, password);
      if (!user) { setError(t('فشل تسجيل الدخول', 'Login failed')); return; }
      const home: Screen =
  ...
  return (
    <div className="auth-screen">
      ...
      <div className="auth-card">
        ...
        <h2 className="auth-heading">{lang === 'ar' ? 'إدخال البيانات' : 'Data Entry'}</h2>
        ...
        <div className="auth-field">
          <input id="login-email" type="email" placeholder={lang === 'ar' ? 'البريد الإلكتروني :' : 'Email Address :'} value={email}
        ...
        <div className="auth-field">
          ...
          <input id="login-password" type={showPass ? 'text' : 'password'} placeholder={lang === 'ar' ? 'كلمة المرور :' : 'Password :'}
            value={password} onChange={e => setPassword(e.target.value)} className="auth-input with-icon" />
        </div>
        <p className="forgot-link">{lang === 'ar' ? 'هل نسيت كلمة السر؟' : 'Forgot Password?'}</p>
        <button
          id="login-submit"
        ...
        >
          {loading ? '...' : (lang === 'ar' ? 'تسجيل الدخول' : 'Login')}
        </button>
        <p className="auth-bottom-link">
          {lang === 'ar' ? 'ليس لديك حساب؟ ' : "Don't have an account? "} <span onClick={() => setScreen('role')}>{lang === 'ar' ? 'قم بالتسجيل الآن' : 'Register Now'}</span>
        </p>
      </div>
    </div>
  );
