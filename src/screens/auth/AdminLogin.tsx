...
export function AdminLogin({ setScreen }: Props) {
  ...
  const handleLogin = async () => {
    if (!email || !password) { setError(lang === 'ar' ? 'يرجى ملء جميع الحقول' : 'Please fill all fields'); return; }
    setLoading(true);
    setError('');
    try {
      const user = await login(email, password);
      if (!user) { setError(lang === 'ar' ? 'بيانات الدخول غير صحيحة' : 'Invalid credentials'); return; }
      if (user.role !== 'admin') { setError(lang === 'ar' ? 'هذا الحساب ليس حساب مدير' : 'This account is not an admin'); return; }
    ...
    <div style={{ height: '100vh', background: GRADIENTS.heroBg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 36 }}>
        ...
        <div>
          ...
          <p style={{ fontFamily: FONT.cairo, fontSize: 12, color: 'rgba(255,255,255,0.7)', margin: 0 }}>{lang === 'ar' ? 'لوحة تحكم المدير' : 'Admin Dashboard'}</p>
        </div>
      </div>

      {/* Card */}
      <div style={{ width: '100%', maxWidth: 400, background: COLORS.card, borderRadius: RADIUS.xxl, padding: '32px 28px', boxShadow: '0 20px 60px rgba(0,0,0,0.25)' }} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
        <h2 style={{ fontFamily: FONT.cairo, fontSize: 22, fontWeight: 800, color: COLORS.textPrimary, margin: '0 0 6px', textAlign: 'center' }}>{lang === 'ar' ? 'تسجيل دخول المدير' : 'Admin Login'}</h2>
        <p style={{ fontFamily: FONT.cairo, fontSize: 13, color: COLORS.textMuted, margin: '0 0 28px', textAlign: 'center' }}>{lang === 'ar' ? 'أدخل بيانات الدخول للمتابعة' : 'Enter credentials to continue'}</p>
        ...
        <div style={{ marginBottom: 16 }}>
          <label style={{ fontFamily: FONT.cairo, fontSize: 13, fontWeight: 700, color: COLORS.textSec, display: 'block', marginBottom: 8 }}>{lang === 'ar' ? 'البريد الإلكتروني' : 'Email Address'}</label>
        ...
        <div style={{ marginBottom: 28 }}>
          <label style={{ fontFamily: FONT.cairo, fontSize: 13, fontWeight: 700, color: COLORS.textSec, display: 'block', marginBottom: 8 }}>{lang === 'ar' ? 'كلمة المرور' : 'Password'}</label>
          <div style={{ display: 'flex', alignItems: 'center', background: COLORS.inputBg, border: `1.5px solid ${COLORS.cardBorder}`, borderRadius: RADIUS.md, padding: '0 14px', gap: 10 }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={COLORS.textMuted} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
        ...
        >
          {loading
            ? <><div style={{ width: 20, height: 20, border: '3px solid rgba(255,255,255,0.3)', borderTopColor: 'white', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} /> {lang === 'ar' ? 'جاري الدخول...' : 'Logging in...'}</>
            : (lang === 'ar' ? 'دخول' : 'Login')
          }
        </button>
        ...
        >
          {lang === 'ar' ? '← العودة لاختيار الدور' : '← Back to Roles'}
        </button>
      </div>
    </div>
  );
