...
const STATUS_MAP: Record<AppointmentStatus, { label: string; bg: string; color: string }> = {
  [AppointmentStatus.Pending]:   { label: 'قيد الانتظار', bg: COLORS.warningLight, color: COLORS.warning },
  [AppointmentStatus.Confirmed]: { label: 'مؤكد',          bg: COLORS.primaryLight,  color: COLORS.primary },
  [AppointmentStatus.Completed]: { label: 'مكتمل',         bg: COLORS.successLight,  color: COLORS.success },
  [AppointmentStatus.Cancelled]: { label: 'ملغي',          bg: COLORS.dangerLight,   color: COLORS.danger  },
  [AppointmentStatus.NoShow]:    { label: 'لم يحضر',       bg: '#FEF3C7',            color: '#92400E'      },
};
      ...
      <div style={{ background: GRADIENTS.heroBg, padding: '28px 20px 32px', flexShrink: 0, borderRadius: `0 0 ${RADIUS.xxl} ${RADIUS.xxl}`, boxShadow: SHADOWS.header }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }} dir="rtl">
            <div>
              <p style={{ fontFamily: FONT.cairo, fontSize: 13, color: 'rgba(255,255,255,0.75)', margin: 0, textAlign: 'right' }}>مرحباً</p>
              <h2 style={{ fontFamily: FONT.cairo, fontSize: 20, fontWeight: 800, color: 'white', margin: '2px 0 0', textAlign: 'right' }}>د. {doctorName}</h2>
            </div>
            <img
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 20 }}>
          {[
            { label: 'مواعيد اليوم', value: appts.length },
            { label: 'مكتملة', value: appts.filter(a => a.status === AppointmentStatus.Completed).length },
            { label: 'قيد الانتظار', value: appts.filter(a => a.status === AppointmentStatus.Pending || a.status === AppointmentStatus.Confirmed).length },
          ].map((s, i) => (
      ...
      <div style={{ flex: 1, padding: '20px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {!loading && appts.length === 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', paddingTop: 40, gap: 12 }}>
            ...
            <p style={{ fontFamily: FONT.cairo, color: COLORS.textMuted, fontSize: 15, fontWeight: 600 }}>لا توجد مواعيد اليوم</p>
          </div>
        )}
          ...
          return (
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                <p style={{ fontFamily: FONT.cairo, fontSize: 13, color: COLORS.textMuted, margin: '0 0 6px' }}>{formatTime(a.date, 'ar-EG')}</p>
    ...
    <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ background: GRADIENTS.heroBg, padding: '24px 20px 28px', flexShrink: 0, borderRadius: `0 0 ${RADIUS.xxl} ${RADIUS.xxl}`, boxShadow: SHADOWS.header }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <SettingsDropdown setScreen={setScreen} />
          <div dir="rtl">
            <p style={{ fontFamily: FONT.cairo, fontSize: 13, color: 'rgba(255,255,255,0.75)', margin: 0 }}>الحالات المنتهية</p>
            <h2 style={{ fontFamily: FONT.cairo, fontSize: 22, fontWeight: 800, color: 'white', margin: '2px 0 0' }}>السجل الطبي</h2>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', background: 'rgba(255,255,255,0.18)', borderRadius: RADIUS.lg, padding: '4px 8px 4px 4px', border: '1.5px solid rgba(255,255,255,0.28)' }} dir="rtl">
          <input
            type="text" placeholder="ابحث باسم المريض..." value={search}
      ...
      <div style={{ flex: 1, padding: '16px 16px 8px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {!loading && filtered.length === 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', paddingTop: 40, gap: 12 }}>
            ...
            <p style={{ fontFamily: FONT.cairo, color: COLORS.textMuted, fontSize: 15, fontWeight: 600 }}>لا يوجد سجلات</p>
          </div>
        )}

        {!loading && filtered.length > 0 && (
          <>
            <p style={{ fontFamily: FONT.cairo, fontSize: 13, color: COLORS.textMuted, margin: '0 4px 4px' }} dir="rtl">{filtered.length} حالة مكتملة</p>
            {filtered.map(a => (
              <button
                key={a.id}
                onClick={() => onSelectPatient({ id: a.patientId, name: a.patientName, phone: a.patientPhone, email: a.patientUserId, num: a.number })}
                style={{
                  background: COLORS.card, border: `1.5px solid ${COLORS.cardBorder}`,
                  borderRadius: RADIUS.xl, padding: '14px 16px',
                  display: 'flex', alignItems: 'center', gap: 14,
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <p style={{ fontFamily: FONT.cairo, fontSize: 16, fontWeight: 800, color: COLORS.textPrimary, margin: '0 0 3px' }}>{a.patientName}</p>
                  <p style={{ fontFamily: FONT.cairo, fontSize: 13, color: COLORS.textMuted, margin: 0 }}>{formatDate(a.date, 'ar-EG')} · {formatTime(a.date, 'ar-EG')}</p>
                </div>
...
function ProfileTab({ setScreen }: { setScreen: (s: Screen) => void }) {
  ...
  const infoRows = [
    { label: 'التخصص',    value: specialty, icon: <svg width="18" height="18" viewBox="0 0 24 24" fill={COLORS.primary}><path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-7 3c1.93 0 3.5 1.57 3.5 3.5S13.93 13 12 13s-3.5-1.57-3.5-3.5S10.07 6 12 6zm7 13H5v-.23c0-.62.28-1.2.76-1.58C7.47 15.82 9.64 15 12 15s4.53.82 6.24 2.19c.48.38.76.97.76 1.58V19z"/></svg>, bg: COLORS.primaryLight },
    { label: 'المستشفى',  value: hospital,  icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#7C3AED" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>, bg: '#EDE9FE' },
    { label: 'العنوان',   value: address,   icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#D97706" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13S3 17 3 10a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>, bg: '#FEF3C7' },
    { label: 'أيام العمل', value: days,     icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={COLORS.success} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>, bg: COLORS.successLight },
    { label: 'الهاتف',    value: phone,     icon: <svg width="18" height="18" viewBox="0 0 24 24" fill={COLORS.primary}><path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z"/></svg>, bg: COLORS.primaryLight },
    { label: 'البريد',    value: email,     icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={COLORS.primary} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>, bg: COLORS.primaryLight },
  ];
      ...
      {modal === 'menu' && (
        <div onClick={() => setModal('none')} style={{ position: 'fixed', inset: 0, zIndex: 900, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
          <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: 480, background: COLORS.card, borderRadius: `${RADIUS.xxl} ${RADIUS.xxl} 0 0`, padding: '28px 24px 40px', display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <p style={{ fontFamily: FONT.cairo, fontSize: 19, fontWeight: 800, color: COLORS.textPrimary, textAlign: 'center', margin: '0 0 4px' }}>الصورة الشخصية</p>
            <button onClick={() => setModal('viewer')} style={{ display: 'flex', alignItems: 'center', gap: 14, background: COLORS.primaryLight, border: `1.5px solid ${COLORS.primary}`, borderRadius: RADIUS.md, padding: '14px 18px', cursor: 'pointer', width: '100%' }}>
              ...
              <span style={{ fontFamily: FONT.cairo, fontSize: 17, fontWeight: 700, color: COLORS.primary }}>عرض الصورة</span>
            </button>
            <button onClick={() => { setModal('none'); fileInputRef.current?.click(); }} style={{ display: 'flex', alignItems: 'center', gap: 14, background: COLORS.card, border: `1.5px solid ${COLORS.cardBorder}`, borderRadius: RADIUS.md, padding: '14px 18px', cursor: 'pointer', width: '100%' }}>
              ...
              <span style={{ fontFamily: FONT.cairo, fontSize: 17, fontWeight: 700, color: COLORS.textPrimary }}>تغيير الصورة</span>
            </button>
            <button onClick={() => setModal('none')} style={{ background: 'transparent', border: 'none', fontFamily: FONT.cairo, fontSize: 16, fontWeight: 600, color: COLORS.textMuted, cursor: 'pointer', padding: '8px' }}>إلغاء</button>
          </div>
        </div>
      )}
      ...
      {showLogoutConfirm && (
        <div onClick={() => setShowLogoutConfirm(false)} style={{ position: 'fixed', inset: 0, zIndex: 900, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: COLORS.card, borderRadius: RADIUS.xl, padding: '28px 24px', width: '100%', maxWidth: 340, textAlign: 'center' }}>
            <p style={{ fontFamily: FONT.cairo, fontSize: 18, fontWeight: 800, color: COLORS.textPrimary, margin: '0 0 8px' }}>تسجيل الخروج</p>
            <p style={{ fontFamily: FONT.cairo, fontSize: 14, color: COLORS.textMuted, margin: '0 0 24px' }}>هل أنت متأكد من تسجيل الخروج؟</p>
            <div style={{ display: 'flex', gap: 12 }}>
              <button onClick={() => setShowLogoutConfirm(false)} style={{ flex: 1, padding: '14px', background: COLORS.card, color: COLORS.textPrimary, border: `1.5px solid ${COLORS.cardBorder}`, borderRadius: RADIUS.md, fontFamily: FONT.cairo, fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>إلغاء</button>
              <button onClick={handleLogout} style={{ flex: 1, padding: '14px', background: COLORS.danger, color: 'white', border: 'none', borderRadius: RADIUS.md, fontFamily: FONT.cairo, fontSize: 15, fontWeight: 800, cursor: 'pointer' }}>خروج</button>
            </div>
          </div>
        </div>
      )}
...
══════════════════════════════════════════════════════════════ */
const NAV: { tab: Tab; labelAr: string; icon: (active: boolean) => React.JSX.Element }[] = [
  {
    tab: 'profile',
    labelAr: 'حسابي',
